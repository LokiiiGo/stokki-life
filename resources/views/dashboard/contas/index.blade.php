...
@section('content')
    ...
    @foreach ($clientes as $cliente)
        <div class="mb-6 border border-stokki-gray-border rounded-lg shadow-sm">
            ...
            <!-- Conteúdo expansível -->
            <div id="cliente-{{ $cliente->id }}" class="hidden bg-white p-4 transition-all duration-500 ease-in-out opacity-0">
                        ...
                        @forelse ($cliente->vendas as $venda)
                            <tr class="border-b border-stokki-gray-border">
                                ...
                                <td class="px-4 py-2">
                                    @if($venda->conta)
                                        <span
                                            class="{{ $venda->conta->status === 'Pago' ? 'text-stokki-green-dark font-bold' : 'text-stokki-red font-bold' }}">
                                ...
                                <td class="px-4 py-2">
                                    @if($venda->conta && $venda->conta->status !== 'Pago')
                                        <form action="{{ route('contas.update', $venda->id) }}" method="POST">
                                            ...
                                            @method('PUT')
                                            <button type="submit"
                                                class="bg-stokki-green text-white px-3 py-1 rounded hover:bg-stokki-green-dark">
                                                Marcar como Pago
                                            </button>
                                        </form>
