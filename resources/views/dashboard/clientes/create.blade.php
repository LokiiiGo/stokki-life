...
@section('content')
    ...
    <!-- O usuário poderá cadastrar um novo cliente -->
    ...
    <!-- Se o cadastro de usuário não carrega, retornará um erro -->
    @if ($errors->any())
    ...
    <form method="POST" action="{{ route('clientes.store') }}" class="bg-white p-6 rounded-lg shadow space-y-6">
        ...
        <div>
            ...
            <input type="text" name="nome" id="nome" value="{{ old('nome') }}" required
                class="w-full border border-stokki-gray-border rounded px-4 py-2" placeholder="Digite nome e sobrenome">
        </div>

        {{-- Número do Cliente (opcional) --}}
        <div>
            <label for="numero_cliente" class="block text-sm font-medium text-stokki-gray-text mb-1">Número do
                Cliente</label>
